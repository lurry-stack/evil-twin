/*
# Withdrawal requirements: min_withdraw, arrival time, holder name

## Changes
1. Add min_withdraw (default 2000) and withdraw_arrival_hours (default 48) to settings
2. Add holder_name column to withdrawals (user's real name for payment)
3. Update request_withdrawal RPC to accept holder name and enforce min_withdraw
*/

ALTER TABLE public.settings ADD COLUMN IF NOT EXISTS min_withdraw numeric NOT NULL DEFAULT 2000;
ALTER TABLE public.settings ADD COLUMN IF NOT EXISTS withdraw_arrival_hours integer NOT NULL DEFAULT 48;

ALTER TABLE public.withdrawals ADD COLUMN IF NOT EXISTS holder_name text;

UPDATE public.settings SET
  min_withdraw = 2000,
  withdraw_arrival_hours = 48
WHERE id = (SELECT id FROM public.settings LIMIT 1);

-- Update request_withdrawal to accept holder name + enforce min_withdraw
...
BEGIN
  ...
  IF p_amount < v_min THEN
    RAISE EXCEPTION 'Minimum withdrawal is % RWF', v_min;
  END IF;

  -- Must own at least one VIP product
  SELECT EXISTS(SELECT 1 FROM public.user_vips WHERE user_id = v_user_id) INTO v_has_vip;
  IF NOT v_has_vip THEN
    RAISE EXCEPTION 'You must purchase at least one product before withdrawing';
  END IF;
